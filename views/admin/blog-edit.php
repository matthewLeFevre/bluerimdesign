...
<?php include $_SERVER['DOCUMENT_ROOT'] . '/modules/header.php'?>
  ...
  <section class="bs-s__ribbon brd-black-white">
      <h1 class="bs-hed bs-lrg">Edit <?= $blog['titleWeb']?></h1>
    </section>
    ...
    <section class="bs-s__lrg">
      <form class="bs-form__lrg bs-center-margin" method="POST" action="/controllers/ctrl-posts.php">
        <div class="bs-form--set">
          ...
          <input name="titleBlog" class="bs-ipt__txt__full bs-main" type="text" value="<?= $blog['titleBlog'] ?>">
        </div>
        <div class="bs-form--set">
          <label for="" class="bs-lbl__txt bs-mid">Blog Web Title</label>
          <input name="titleWeb" class="bs-ipt__txt__full bs-main" type="text" value="<?= $blog['titleWeb'] ?>">
        </div>
        <div class="bs-form--set">
          <label for="" class="bs-lbl__txt bs-mid">Summary</label>
          <textarea name="summary" class="bs-ipt__area__sml bs-main" type="text"><?= $blog['summary']?></textarea>
        </div>
        <div class="bs-form--set">
          <label for="" class="bs-lbl__txt bs-mid">Blog Post Body</label>
          <textarea name="markup" class="bs-ipt__area__lrg bs-main" type="text"><?= $blog['markup']?></textarea>
        </div>
        <div class="bs-form--set">
          <label for="" class="bs-lbl__txt bs-mid">Youtube Video Embed</label>
          <textarea name="videoEmbed" class="bs-ipt__area__sml bs-main" type="text"><?= $blog['videoEmbed']?></textarea>
        </div>
        <div class="bs-form--set">
          <label for="" class="bs-lbl__txt bs-mid">Youtube Video Share Link</label>
          <input name="videoUrl" class="bs-ipt__txt__full bs-main" type="text" value="<?= $blog['videoUrl'] ?>">
        </div>
        <div class="bs-form--set">
          <label for="" class="bs-lbl__txt bs-mid">Path to Icon</label>
          <input name="iconPath" class="bs-ipt__txt__full bs-main" type="text" value="<?= $blog['iconPath'] ?>">
        </div>
        <div class="bs-form--set__align-right">
          ...
          <?php foreach($topics as $topic) { ?>
            <option <?php if($topic['id'] == $blog['id']){ echo('selected ');} ?> value=<?= $topic['id'] ?>><?= $topic['name']?></option>
          <?php }?>
          </select>
        </div>
        <div class="bs-form--set__align-right">
        ...
        <div class="bs-form--set">
          ...
          <input type="hidden" name="action" value="post_edit_process">
        </div>
      </form>
    </section>
	</main>
